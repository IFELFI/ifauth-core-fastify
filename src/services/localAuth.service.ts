...
export class LocalAuthService {
   ...
   * @returns Token pair with access and refresh tokens
   */
  public async login(loginData: Static<typeof localLoginSchema.body>): Promise<TokenPair> {
    return await this.#fastify.prisma.$transaction(async (tx) => {
      const searchUser = await tx.users.findUnique({ where: { email: loginData.email } });
      if (!searchUser) {
        throw this.#fastify.httpErrors.unauthorized("Invalid email or password");
      }
      const searchPassword = await tx.password.findUnique({ where: { user_id: searchUser.id } });
      if (!searchPassword) {
        throw this.#fastify.httpErrors.unauthorized("Invalid email or password");
      }
      // Compare password
      const comparePassword = await bcrypt.compare(loginData.password, searchPassword.password);
      if (!comparePassword) {
        throw this.#fastify.httpErrors.unauthorized("Invalid email or password");
      }
      const profile = await tx.profile.findUnique({ where: { user_id: searchUser.id } });
      if (!profile) {
        throw this.#fastify.httpErrors.internalServerError("Error finding user profile");
      }
